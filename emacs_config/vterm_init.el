(unless (or (eq system-type 'windows-nt) (eq system-type 'ms-dos))
  ...
  (unless module-file-suffix
    (unless (f-exists? (nomon:concat_config_dir ".locks/vterm_init_module_support_warning.lk"))
      (read-string "VTerm needs module support.  Please compile Emacs with  the --with-modules option!")
      (f-touch (nomon:concat_config_dir ".locks/vterm_init_module_support_warning.lk"))
      )
    )
  (when module-file-suffix
    (unless (executable-find "cmake")
      (unless (f-exists? (nomon:concat_config_dir ".locks/vterm_init_cmake_warning.lk"))
	(read-string "You don't have Cmake in your system thus vterm will not be installed")
	(f-touch (nomon:concat_config_dir ".locks/vterm_init_cmake_warning.lk"))
	)
      )
    (unless (executable-find "libtool")
      (unless (f-exists? (nomon:concat_config_dir ".locks/vterm_init_libtool_warning.lk"))
	(read-string "You don't have libtool-bin in your system thus vterm will not be installed")
	(f-touch (nomon:concat_config_dir ".locks/vterm_init_libtool_warning.lk"))
	)
      )
    )
  )
